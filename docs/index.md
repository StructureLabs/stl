...
[**link to rivt user manual**](https://github.com/ShareCalcs/rivtlib)

[**link to rivt installers**](https://github.com/ShareCalcs/rivtlib)

## Introduction

Calculations and drawings are road maps for building engineering designs. Recent
progress in open source software have provided general tools that can be used to
improve their clarity, completeness and timeliness. Most importantly they
provide an opportunity to reduce barriers to sharing and collaboration on
calculations. 

Sharing is important because many engineering technologies are not changing.
They are often fixed and codified for decades. Most of the hard work lies in
developing good understanding of many established technologies and then
skillfully and efficiently adapting and combining them to meet the needs of a
particular project. Calculations reflect and record this process. The
professions collectively have an opportunity to produce large, shared general
calculation libraries that can be reused and recombined as needed on a project
basis. This model of shared incremental improvement, using open source licenses
and tools, is a strong, proven paradigm for engineering progress. 

A number of powerful calculation programs exist, but they present barriers to
sharing. In many cases the barrier is the high initial and recurring software
costs. Costs are further increased by constantly changing proprietary file
formats that require upgrades or subscriptions, and by file incompatibility
between different programs. Often the software does not produce collated
reports, or easily include background information needed for clear communication
and commercial distribution of designs. And they may not be available on all
operating systems. Taken together these barriers prevent efficiencies and
quality improvements that come from widespread sharing. They result in
essentially identical calculations being rewritten thousands of times.

**rivt** is an open source software stack that lowers these barriers.


## **rivt** Overview

**rivt** is an integration of three open source programs that implements
**rivtText**, a plain text, human readable, calculation markup language built on
restructuredText.  **rivtlib** is the Python library that processes
**rivtText**. **Github** repositories are the primary platform used to search
and distribute **rivt** calcs.

**rivt** can be installed on the desktop or run in the cloud.  The minimum
software needed (for text output) is Python and a plain text processor. The
basic installation required for efficient workflow and formal document
production requires: 
...
Installation programs are provided here - [**link to rivt**](https://github.com/ShareCalcs/rivt)


## **rivtlib** Overview

**rivtlib** is a Python package that provides a calculation API for **rivtText**.
The API uses file and folder naming conventions The
